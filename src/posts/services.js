...
//angular.module('window.visionindustry.services', [])
...
subscription.revisionindustry = true;
}        
...
$http.put(API_END_POINT+"/api/devices", {deviceid: deviceid, username: username, subscription: subscription, chain: rootScope.$storage.chain});
      ...
      search: function(text) {
        return $http.get("https://api.askvisionindustry.com/search?q="+text+"&include=meta&sort_by=created&order=desc");
      },
  ...
  app.filter('catchimage', function(){
    return function(inp) {
      ...
      if (x1 && x1.length>1){
        return "https://newmedia.vit.tube/resize/640x480/"+x1[1].split('"')[1];
      } else {
        /*if (x1.length == 1) {
  ...
  app.filter('pkeys', function(rootScope){
    return function(inp,role){
      if (inp) {
        ...
        return window.visionindustry.auth.toWif(inp.name, inp.password, role);
      }
    }
  });
  ...
  app.filter('readingtime', function($sce, rootScope) {
	...
	app.filter('sp', function($sce, rootScope) {
	    return function(text) {
	    	if (text) {
			console.log(text);
	    		return (Number(text.split(" ")[0])/1e6*rootScope.$storage.visionindustry_per_mvests).toFixed(3);
	    	}
	    };
	})
	app.filter('sd', function($sce, rootScope) {
	    return function(text, balance, sbd) {
			...
			//console.log(rootScope.$storage);
	    		//return ((Number(text.split(" ")[0])/1e6*rootScope.$storage.visionindustry_per_mvests*rootScope.$storage.base + Number(balance.split(" ")[0])*rootScope.$storage.base + Number(sbd.split(" ")[0])).toFixed(3))*rootScope.$storage.currencyRate;
	    	}
	    };
	})
              ...
              attrs.$observe('ngSrc', function(ngSrc) {
                if (angular.isDefined(ngSrc)) {
                  $http.get(ngSrc).success(function(){
                      ...
                      if (ngSrc && rootScope.chain && rootScope.chain == 'visionindustry') {
                  ...
                  $scope.toggleComment = function(comment) {
                      ...
                      if (comment.showChildren) {
                        ...
                        if (comment.depth % 5 == 0) {
                          ...
                          window.visionindustry.api.getStateAsync('tag/@'+comment.author+'/'+comment.permlink, function(err, dd) {
                            //console.log(dd);
                            var po = [];
                  ...
                  $scope.reply = function (xx) {
                    ...
                    if (!$scope.editc) {
                        ...
                        if (rootScope.user) {
                          ...
                          var json = {tags: angular.fromJson($scope.post.json_metadata).tags[0] || ["evisionindustry"], app: 'evisionindustry/'+rootScope.$storage.appversion, format: 'markdown+html', community: 'evisionindustry' };                              
                    ...
                    confirmPopup.then(function(res) {
                        if(res) {
                            ...
                            if (rootScope.user) {
                              ...
                              window.visionindustry.broadcast.deleteComment(wif, comment.author, comment.permlink, function(err, result) {
                                //console.log(err, result);
                                if (err) {
                                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                                  rootScope.showAlert($filter('translate')('ERROR'), $filter('translate')('BROADCAST_ERROR')+" "+message)
    ...
    function ius($q, $ionicLoading, $cordovaFileTransfer, $ionicPlatform, $filter, rootScope, API_END_POINT) {
        ...
        function uploadImage(imageURI) {
          ...
          function uploadFile() {
            ...
            // "https://api.cloudinary.com/v1_1/evisionindustry/image/upload"
            var uploadOptions = {
