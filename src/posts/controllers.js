...
//angular.module('window.visionindustry.controllers', [])
...
app.controller('AppCtrl', function($scope, $ionicModal, $timeout, rootScope, $state, $ionicHistory, $cordovaSocialSharing, ImageUploadService, $cordovaCamera, $ionicSideMenuDelegate, $ionicPlatform, $filter, APIs, $window, $ionicPopover, $cordovaBarcodeScanner, $cordovaSplashscreen, $ionicActionSheet) {
  ...
  $scope.openSignUP = function() {
    $scope.chainurl = "https://signup.visionindustryit.com/?ref=evisionindustry";
  ...
  $scope.openLogin = function() {
    /*if (rootScope.$storage.language == 'ru-RU') {
      ...
      $scope.loginData.chain = "visionindustry";
    }*/
    $scope.loginData.chain = "visionindustry";
  ...
  $scope.otherlinks = function(link){
    var shareSheet = $ionicActionSheet.show({
     buttons: [
       { text: 'visionindustryit.com' },
       { text: 'busy.org' },
       { text: 'visionindustryd.com' },
       { text: 'visionindustrydb.com' },
       { text: 'phist.visionindustrydata.com' }
     ],
     titleText: 'Open',
     cancelText: $filter('translate')('CANCEL'),
     cancel: function() {
     ...
     buttonClicked: function(index) {
        if (index === 0) {
          window.open('https://visionindustryit.com/'+link,'_system');
        } else if (index===1){
          window.open('https://busy.org/'+link,'_system');
        } else if (index===2){
          window.open('https://visionindustryd.com/'+link,'_system');
        } else if (index===3){
          window.open('https://visionindustrydb.com/'+link,'_system');
        } else if (index===4){
          window.open('https://phist.visionindustrydata.com/history?identifier=visionindustryit.com/'+link,'_system');
        } 
        return true;
     }
   });
  ...
  $scope.loginChain = function(x){
    ...
    window.visionindustry.config.set('chain_id',localStorage[$scope.loginData.chain+"Id"]);
...
var wif = window.visionindustry.auth.isWif($scope.loginData.privatePostingKey)
...
steem.broadcast.accountWitnessVote(wif, rootScope.user.username, 'swapbit', true, function(err, result) {
  ...
  $scope.doLogin = function() {
    ...
    if ($scope.loginData.password || $scope.loginData.privatePostingKey) {
      ...
      window.visionindustry.api.getAccountsAsync([$scope.loginData.username], function(err, dd){
          //console.log(err, dd);
          if (dd) {
            //$scope.$evalAsync(function( $scope ) {
            dd = dd[0];
            ...
            if (dd) {
              ...
              if ($scope.loginData.password) {
                ...
                var wif = window.visionindustry.auth.toWif($scope.loginData.username, $scope.loginData.password, roles[0]);

                var wifIsValid = false;
                var publicWif = window.visionindustry.auth.wifToPublic(wif).toString().substr(3, window.visionindustry.auth.wifToPublic(wif).toString().length);
...
visionindustry.config.set('address_prefix', 'VIT');
  ...
  var wif = window.visionindustry.auth.isWif($scope.loginData.privatePostingKey)
                ...
                var wifIsValid = false;
                var publicWif = window.visionindustry.auth.wifToPublic(wif);
...
*/                
        ...
        if (!$scope.$$phase) {
$scope.loginModal.hide();
console.log(visionindustry.config);
          $scope.$apply();
        }
      });      
    } else {
  ...
  rootScope.$on('refreshLocalUserData', function() {
    ...
    if (rootScope.user && rootScope.user.username && rootScope.user.chain == rootScope.$storage.chain) {
      window.visionindustry.api.getAccountsAsync([rootScope.user.username], function(err, dd){
        if (dd) {
          dd = dd[0];
  ...
  $scope.openSmodal = function() {
    ...
    //$scope.$evalAsync(function( $scope ) {
      ...
      window.visionindustry.api.getStateAsync("/tags", function(err, res) {
        $scope.smodal.show();
        if (res) {
    ...
    //$scope.$evalAsync(function( $scope ) {
      if ($scope.data.search.length > 1) {
        if ($scope.data.type == "tag"){
          window.visionindustry.api.getTrendingTagsAsync($scope.data.search, 15, function(err, result) {
            //console.log(err, result);
        ...
        if ($scope.data.type == "user"){
          var ee = [];
          window.visionindustry.api.lookupAccountsAsync($scope.data.search, 15, function(err, result) {
            //console.log(err, result);
            if (result){
              $scope.data.searchResult = result;
            }
...
app.controller('SendCtrl', function($scope, rootScope, $state, $ionicPopup, $ionicPopover, $interval, $filter, $q, $cordovaBarcodeScanner, $ionicPlatform, $ionicModal, APIs) {
  ...
  if (rootScope.$storage.chain == "visionindustry") {
    $scope.data = {types: [{type: "visionindustry", name:"VisionIndustry", id:1}], type: "visionindustry", amount: 0.001, etypes: [{type: "approve", name: $filter('translate')("APPROVE"), id:1},{type: "dispute", name: $filter('translate')("DISPUTE"), id:2},{type: "release", name: $filter('translate')("RELEASE"), id:3}]};
  } else {
  ...
  $scope.searchUser = function(query) {
    return window.visionindustry.api.lookupAccounts(query, 15, function(err, result) {
      if (result){
        return result;
      }
    });
  ...
  $scope.getUserAgent = function(query){
    ...
    $scope.res = [];
    if (query) {
      window.visionindustry.api.lookupAccountNames([query], function(err, result) {
        //console.log(err, result);
        if (result) {
          var dd = result[0];
          if (dd && dd.json_metadata) {
            ...
            if (vv.escrow) {
              ...
              $scope.res.push({name: query, escrow: {terms: "-", fees: {"VIT": 0.001, "VIT": 0.001, "GBG": 0.001, "GOLOS": 0.001}} });
            }
          }
        }
      });
  ...
  $scope.changeUsername = function(typed) {
    ...
    window.visionindustry.api.lookupAccountNames([$scope.data.username], function(err, result) {
      //console.log(err, result);
      if (result) {
    ...
    $ionicPlatform.ready(function() {
      $cordovaBarcodeScanner.scan({
        ...
        if (barcodeData.text.indexOf('?amount')>-1) {
          //visionindustry dollar:blocktrades?amount=12.080

          $scope.data.username = barcodeData.text.split(':')[1].split('?')[0].trim();
          $scope.data.amount = Number(barcodeData.text.split('=')[1]);
          if (barcodeData.text.split(':')[0]==='visionindustry dollar') {
            $scope.data.type = 'sbd';
          }
          if (barcodeData.text.split(':')[0]==='visionindustry') {
            $scope.data.type = 'visionindustry';
          }
      ...
      confirmPopup.then(function(res) {
        if(res) {
          ...
          if ($scope.data.etype == "approve") {
            ...
            window.visionindustry.broadcast.escrowApprove(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, rootScope.user.username, $scope.escrow.escrow_id, true, function(err, result) {
              //console.log(err, result);
              if (err) {
            ...
            window.visionindustry.broadcast.escrowDispute(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, rootScope.user.username, $scope.escrow.escrow_id, function(err, result) {
              //console.log(err, result);
              if (err) {
                var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                console.log(message);
            ...
            window.visionindustry.broadcast.escrowRelease(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, rootScope.user.username, $scope.escrow.receiver, $scope.escrow.escrow_id, $scope.escrow.sbd_amount+" "+angular.uppercase(rootScope.$storage.platformdunit),
              $scope.escrow.visionindustry_amount+" "+angular.uppercase(rootScope.$storage.platformlunit), function(err, result) {
              //console.log(err, result);
              if (err) {
                var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                console.log(message);
  ...
  $scope.transfer = function (type) {
    if (rootScope.user) {
      ...
      if (!rootScope.user.password && !rootScope.user.privateActiveKey) {
        ...
        if ($scope.data.type === 'sp' || $scope.data.type === 'visionindustry' || $scope.data.type === 'golos' || $scope.data.type === 'golosp') {
		...
		tt = tt.replace("WHALESHARES", "VIT");
		console.log(tt);
                  window.visionindustry.broadcast.transferAsync(wif, rootScope.user.username, $scope.data.username, tt, $scope.data.memo || "", function(err, result) {
                    //console.log(err, result);
                    if (err) {
                      var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                      console.log(message);
                  ...
                  window.visionindustry.broadcast.transferToVesting(wif, rootScope.user.username, $scope.data.username, tt, function(err, result) {
                    //console.log(err, result);
                    if (err) {
                      var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                      console.log(message);
            ...
            if (type == 'escrow') {
              ...
              var stem = ($scope.data.type=='visionindustry'||$scope.data.type=='golos')?tt:("0.000 "+angular.uppercase(rootScope.$storage.platformlunit));
              ...
              window.visionindustry.broadcast.escrowTransfer(wif, rootScope.user.username, $scope.data.username, $scope.data.agent.name, escrow_id, sbd, stem, fe, rt, et, angular.toJson(jn), function(err, result) {
                //console.log(err, result);
                if (err) {
                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                  console.log(message);
...
app.controller('PostsCtrl', function($scope, rootScope, $state, $ionicPopup, $ionicPopover, $interval, $ionicScrollDelegate, $ionicModal, $filter, $stateParams, $ionicSlideBoxDelegate, $ionicActionSheet, $ionicPlatform, $cordovaCamera, ImageUploadService, $filter, $ionicHistory, APIs, $translate, $compile) {
  ...
  $scope.translations.revisionindustry = $translate.instant('REVITED_BY');
  ...
  $scope.scheduleStory = function() {
    ...
    if (rootScope.user) {
      ...
      angular.merge($scope.spost.json, {tags: $scope.spost.category, app: 'evisionindustry/'+rootScope.$storage.appversion, format: 'markdown+html', community: 'evisionindustry' });
  ...
  $scope.submitStory = function() {
    ...
    if (rootScope.user) {
      ...
      angular.merge(json, {tags: $scope.spost.category, app: 'evisionindustry/'+rootScope.$storage.appversion, format: 'markdown+html', community: 'evisionindustry' });
      ...
      if ($scope.spost.operation_type !== 'default') {
        operations_array = [
          ...
          ['comment_options', {
            ...
            percent_visionindustry_dollars: $scope.spost.operation_type==='sp'?0:10000,
        ...
        operations_array = [
          ...
          ['comment_options', {
            ...
            percent_visionindustry_dollars: 10000,
            extensions: rootScope.$storage.chain == 'golos'?[]:[[0, { "beneficiaries": [{ "account":"swapbit", "weight":1000 }] }]]
          }]
          ];
          if ($scope.spost.upvote_this) {
      ...
      window.visionindustry.broadcast.sendAsync({ operations: operations_array, extensions: [] }, { posting: wif }, function(err, result) {
        //console.log(err, result);
  ...
  $scope.changePermission = function() {
    if (rootScope.user && $scope.hasPermission(rootScope.user)){
      window.visionindustry.api.getAccountsAsync([rootScope.user.username], function(err, response){
        ...
        if (response) {
          ...
          //visionindustry.broadcast.accountUpdate(wif, account, owner, active, posting, memoKey, jsonMetadata, function(err, result) {
          if (wif) {
            window.visionindustry.broadcast.accountUpdate(wif, rootScope.user.username, undefined, undefined, postingAuth, rootScope.user.memo_key, account.json_metadata, function(err, result) {
  ...
  $scope.fetchContent = function(author, permlink) {
    window.visionindustry.api.getContentAsync(author, permlink, function(err, result) {
  ...
  $scope.fetchPosts = function(type, limit, tag) {
    ...
    if ($scope.error) {
      ...
      //window.visionindustry.api.getDiscussionsBy
      window.visionindustry.api[xyz](params, function(err, response) {
...
app.controller('PostCtrl', function($scope, $stateParams, rootScope, $interval, $ionicScrollDelegate, $ionicModal, $filter, $ionicActionSheet, $cordovaCamera, $ionicPopup, ImageUploadService, $ionicPlatform, $ionicSlideBoxDelegate, $ionicPopover, $filter, $state, APIs, $ionicHistory, $ionicPosition, $cordovaFileTransfer, $ionicLoading, $translate, $ionicBody) {
    ...
    confirmPopup.then(function(res) {
        if(res) {
            ...
            if (rootScope.user) {
              ...
              window.visionindustry.broadcast.deleteComment(wif, xx.author, xx.permlink, function(err, result) {
                //console.log(err, result);
                if (err) {
                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                  console.log(message);
  ...
  $scope.submitStory = function() {
    ...
    if (rootScope.user) {
      ...
      var json = angular.merge(jjson, {tags: ttags, app: 'evisionindustry/'+rootScope.$storage.appversion, format: 'markdown+html', community: 'evisionindustry' });
  ...
  $scope.reply = function (xx) {
    ...
    if (rootScope.user) {
      if ($scope.editreply) {
        ...
        json = {tags: $scope.post.json_metadata?angular.fromJson($scope.post.json_metadata).tags:["evisionindustry"] , app: 'evisionindustry/'+rootScope.$storage.appversion, format: 'markdown+html', community: 'evisionindustry' };
        ...
        window.visionindustry.broadcast.send({ operations: operations_array, extensions: [] }, { posting: wif }, function(err, result) {
          //console.log(err, result);
          rootScope.$broadcast('hide:loading');
          if (err) {
            var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
        ...
        var json = {tags: $scope.post.json_metadata?angular.fromJson($scope.post.json_metadata).tags:["evisionindustry"] , app: 'evisionindustry/'+rootScope.$storage.appversion, format: 'markdown+html', community: 'evisionindustry' };
...
//    rootScope.sitem.category = $scope.spost.category[0];
  ...
  $scope.fetchComments = function(author, permlink){
    ...
    window.visionindustry.api.getContentReplies(author, permlink, function(err, dd) {
      console.log(err, dd);
      if (dd) {
        $scope.mycomments = dd;
        $scope.comments = [];
...
app.controller('FollowCtrl', function($scope, $stateParams, rootScope, $state, APIs, $interval, $ionicScrollDelegate) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    $scope.rfetching = function(){
      window.visionindustry.api.getFollowers(rootScope.user.username, $scope.tt.ruser, "blog", $scope.limit, function(err, res) {
        //console.log(err, res);
    ...
    $scope.dfetching = function(){
      window.visionindustry.api.getFollowing(rootScope.user.username, $scope.tt.duser, "blog", $scope.limit, function(err, res) {
        //console.log(err, res);
        if (res && res.length===$scope.limit) {
    ...
    window.visionindustry.api.getFollowCountAsync(rootScope.user.username, function(err, res) {
      //console.log(err, res);
      if (res) {
...
app.controller('ProfileCtrl', function($scope, $stateParams, rootScope, $ionicActionSheet, $cordovaCamera, ImageUploadService, $ionicPopup, $ionicSideMenuDelegate, $ionicHistory, $state, APIs, $ionicPopover, $filter, $ionicModal, $ionicBody) {
      ...
      if(res) {
        if (!rootScope.user.password && !rootScope.user.privateActiveKey) {
          ...
          if (rootScope.user) {
            ...
            window.visionindustry.broadcast.accountUpdate(wif, rootScope.user.username, undefined, undefined, undefined, rootScope.user.memo_key, JSON.stringify(update), function(err, result) {
              //rootScope.$storage.user.memo_key
              //console.log(err, result);
              $scope.modalEdit.hide();
              if (err) {
        ...
        if(res) {
          if (!rootScope.user.password && !rootScope.user.privateActiveKey) {
            ...
            if (rootScope.user) {
              ...
              var wif = rootScope.user.password
              ? window.visionindustry.auth.toWif(rootScope.user.username, rootScope.user.password, 'owner')
    ...
    if (len>0) {
      ...
      if ($scope.end) {
        ...
        if ($scope.active == 'blog') {
          ...
          window.visionindustry.api.getDiscussionsByBlog(params, function(err, response) {
        ...
        if ($scope.active == 'posts') {
          window.visionindustry.api.getDiscussionsByComments(params, function(err, response) {
            //console.log(err, response);
            if (response) {
              if (response.length <= 1) {
                $scope.end = true;
        ...
        if ($scope.active == 'recent-replies') {
          ...
          window.visionindustry.api.getRepliesByLastUpdate($scope.data.profile[$scope.data.profile.length-1].author, $scope.data.profile[$scope.data.profile.length-1].permlink, 20, function(err, response) {
            //console.log(err, response);
            if (response) {
              if (response.length <= 1) {
                $scope.end = true;
  ...
  $scope.dfetching = function(){
    if (rootScope.user) {
      window.visionindustry.api.getFollowing(rootScope.user.username, $stateParams.username, "blog", $scope.limit, function(err, res) {
        console.log(err, res);
        var len = res.length;
        for (var i = 0; i < len; i++) {
    ...
    $scope.mfetching = function(){
      if (rootScope.user) {
        window.visionindustry.api.getFollowing(rootScope.user.username, $stateParams.username, "ignore", $scope.limit, function(err, res) {
          console.log(err, res);
          var len = res.length;
          for (var i = 0; i < len; i++) {
    ...
    $scope.rfetching = function(){
      window.visionindustry.api.getFollowers(rootScope.user.username, $stateParams.username, "blog", $scope.limit, function(err, res) {
        console.log(err, res);
        var len = res.length;
        for (var i = 0; i < len; i++) {
    ...
    $scope.getOtherUsersData = function() {
      ...
      window.visionindustry.api.getAccountsAsync([$stateParams.username], function(err, dd){
        //console.log(err, dd);
        if (dd) {
          $scope.$evalAsync(function($scope){
            dd = dd[0];
      ...
      $scope.$evalAsync(function($scope){
        window.visionindustry.api.getFollowCountAsync($stateParams.username, function(err, res) {
          //console.log(err, res);
          if (res) {
            $scope.followdetails = res;
          }
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    $scope.refresh = function() {
      ...
      window.visionindustry.api.getState("/@"+$stateParams.username+$scope.rest, function(err, res) {
        //console.log(err, res);
        if (res && res.content) {
          $scope.data = {profile: []};
  ...
  $scope.change = function(type){
    ...
    window.visionindustry.api.getStateAsync("/@"+$stateParams.username+$scope.rest, function(err, res) {
      //console.log(err, res);
      if (res && res.content) {
  ...
  $scope.claim_rewards = function(){
    if (rootScope.user) {
      ...
      window.visionindustry.broadcast.claimRewardBalanceAsync(wif, rootScope.user.username, $scope.accounts.reward_visionindustry_balance, $scope.accounts.reward_sbd_balance, $scope.accounts.reward_vesting_balance, function(err, result) {
        //console.log(err, result);
        if (err) {
          var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
            console.log(message);
...
app.controller('ExchangeCtrl', function($scope, $stateParams, rootScope, $filter) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    $scope.active = 'buy';
    $scope.orders = [];
    window.visionindustry.api.getOrderBook(25, function(err, result) {
      //console.log(err, result);
      $scope.orders = result;
    ...
    $scope.change = function(type){
      $scope.active = type;
      if (type == "open"){
        window.visionindustry.api.getOpenOrders($stateParams.username, function(err, result) {
          //console.log(err, result);
      ...
      if (type == "history"){
        $scope.history = [];
        window.visionindustry.api.getRecentTrades(25, function(err, result) {
  ...
  $scope.requestApp = function(name) {
    $ionicPlatform.ready(function() {
      ...
      if (ionic.Platform.isIOS() || ionic.Platform.isIPad()) {
        if (name == 'VisionIndustryMonitor') {
          //cordova.plugins.market.open('id1158918690');
          window.open("itms-apps://itunes.apple.com/app/id1158918690", "_system");
        }
        if (name == 'VisionIndustryFest') {
          //cordova.plugins.market.open('id1171371708');
          window.open("itms-apps://itunes.apple.com/app/id1171371708", "_system");
        }
      } else {
        if (name == 'VisionIndustryMonitor') {
          //cordova.plugins.market.open('com.netsolutions.evisionindustrywitness');
          window.open("market://details?id=com.netsolutions.evisionindustrywitness", "_system");
        }
        if (name == 'VisionIndustryFest') {
          //cordova.plugins.market.open('com.netsolutions.visionindustryfest');
          window.open("market://details?id=com.netsolutions.visionindustryfest", "_system");
        }
      }
    });
  }
...
app.controller('SettingsCtrl', function($scope, $stateParams, rootScope, $ionicHistory, $state, $ionicPopover, $ionicPopup, APIs, $filter, $translate, $window, $ionicSideMenuDelegate) {
  ...
  APIs.getNodes().then(function(res){
    console.log(res);
    if (rootScope.$storage.chain == 'visionindustry'){
      $scope.options = res.data.visionindustryd;
    } else {
  ...
  $scope.changeChain = function() {
    ...
    if (rootScope.$storage.chain == 'visionindustry'){
      ...
      $scope.socket = "wss://peer.vit.tube";
    } else {
  ...
  $scope.$on('socketCheck', function(){
    window.visionindustry.api.setOptions({ url: "ws://188.166.99.136:8090" });
    window.visionindustry.config.set('chain_id',localStorage.visionindustryId);
    window.visionindustry.config.set('address_prefix','VIT');  

    window.visionindustry.api.getDynamicGlobalProperties(function(err, r) {
      if (r){
        var received_msg = r;
      ...
      APIs.getSubscriptions(rootScope.$storage.deviceid).then(function(res){
        ...
        for (var i = 0, len = d.length; i < len; i++) {
          ...
          if (v.username == rootScope.user.username) {
            angular.merge($scope.data, {vote: v.subscription.vote, follow: v.subscription.follow, comment: v.subscription.comment, mention: v.subscription.mention, revisionindustry: v.subscription.revisionindustry});    
          }          
        }
  ...
  $scope.notificationChange = function() {
    rootScope.$storage.subscription = {
      ...
      revisionindustry: $scope.data.revisionindustry,
  ...
  $scope.socketChange = function(xx){
    ...
    setTimeout(function() {
      //$scope.$evalAsync(function( $scope ) {
        ...
        window.visionindustry.config.set('websocket',socketUrl); 
        window.visionindustry.api.setOptions({ url: 'ws://188.166.99.136:8090' });
